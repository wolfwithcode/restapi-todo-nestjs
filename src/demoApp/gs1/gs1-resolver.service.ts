...
export class GS1ResolverService {
  ...
  async upsertProduct(productId: string, data: any): Promise<any> {
    ...
    const productData = {
      ...
      createdAt: existingProduct
        ? existingProduct.data.createdAt
        : new Date().toISOString(),
    };

    // Save product data with ETag validation
    const success = await this.gs1Storage.saveProduct(
      productId,
      productData,
      etag,
    );
  ...
  async getProductCertificates(productId: string): Promise<any[]> {
    ...
    if (product.certificateIds && Array.isArray(product.certificateIds)) {
      for (const certId of product.certificateIds) {
        const cert = await this.gs1Storage.getProductCertificate(
          productId,
          certId,
        );
        if (cert) {
  ...
  async getCertificateWithETag(
    ...
    const certificate = await this.gs1Storage.getProductCertificateWithETag(
      productId,
      certificateId,
    );
    if (!certificate) {
  ...
  async addProductCertificate(
    ...
    if (!certificateData.id) {
      certificateData.id = `cert-${Date.now()}-${Math.random()
        .toString(36)
        .substring(2, 9)}`;
    }
    ...
    const updatedCertIds = [
      ...(productWithETag.data.certificateIds || []),
      certificateData.id,
    ];
    ...
    const updatedCert = await this.gs1Storage.getProductCertificateWithETag(
      productId,
      certificateData.id,
    );
    return {
  ...
  async getProductHistory(productId: string): Promise<any[]> {
    ...
    for (const timestamp of timestamps) {
      const record = await this.gs1Storage.getProductHistory(
        productId,
        timestamp,
      );
      if (record) {
  ...
  async upsertCompany(companyId: string, data: any): Promise<any> {
    ...
    const companyData = {
      ...
      id: companyId,
      updatedAt: new Date().toISOString(),
      createdAt: existingCompany
        ? existingCompany.data.createdAt
        : new Date().toISOString(),
    };

    // Save company data with ETag validation
    const success = await this.gs1Storage.saveCompany(
      companyId,
      companyData,
      etag,
    );
