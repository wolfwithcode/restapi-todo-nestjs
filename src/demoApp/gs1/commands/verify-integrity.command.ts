...
export class VerifyIntegrityCommand extends CommandRunner {
  ...
  @Option({
    flags: '-i, --id <entityId>',
    description:
      'Entity ID (e.g. product ID or company ID, use "system" for metadata)',
    required: true,
  })
  ...
  async run(
    ...
    try {
      this.logger.log(
        `Verifying integrity of ${options.entityType} with ID: ${options.entityId}`,
      );
      ...
      console.log(
        `Concurrency Control: ${
          result.concurrencyControlReady ? 'READY ✓' : 'NOT READY ✗'
        }`,
      );
      ...
      if (!result.concurrencyControlReady) {
        ...
        console.log(
          'This may lead to potential data corruption in concurrent update scenarios.',
        );
      } else {
        console.log('\n✓ Entity supports ETag-based concurrency control.');
        console.log(
          'Update operations will be protected against concurrent modifications.',
        );
      }
    } catch (error: unknown) {
      const errorMessage =
