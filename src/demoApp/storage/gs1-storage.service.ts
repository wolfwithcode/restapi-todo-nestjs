...
export class GS1StorageService {
  ...
  async getProductCertificate(
    ...
    const key = this.getObjectKey(
      `products/${productId}/certificates/${certificateId}.json`,
    );
    const result = await this.minioService.getFileWithETag(key);
    return result ? result.data : null;
  }
   ...
   */
  async getProductCertificateWithETag(
    productId: string,
    certificateId: string,
  ): Promise<{ data: any; etag: string } | null> {
    const key = this.getObjectKey(
  ...
  async addProductHistory(productId: string, data: any): Promise<boolean> {
    ...
    const key = this.getObjectKey(
      `products/${productId}/history/${timestamp}.json`,
    );
   ...
   */
  async getSystemMetadataWithETag(): Promise<{
    data: any;
    etag: string;
  } | null> {
    const key = this.getObjectKey('metadata/last_updated.json');
  ...
  async initializeStructure(): Promise<boolean> {
    try {
      ...
      const metadataResult = await this.minioService.uploadFile(
        metadataKey,
        systemMetadata,
      );
      ...
      const indexResult = await this.minioService.uploadFile(
        indexKey,
        productIndex,
      );
      ...
      this.logger.log(
        'GS1 identity resolver structure initialized with ETag-based concurrency control',
      );
      return true;
    } catch (error) {
      this.logger.error(
        `Failed to initialize GS1 structure: ${
